...
`humanize-monolith` 에이전트가 한 콜에서 탐지·윤문·자체검증을 끝내기 위해 사용하는 슬림 룰북.
본진 `ai-tell-taxonomy.md`(590줄)에서 S1·S2 핵심 패턴만 추려 처방과 함께 한 줄로 압축했다.
...
| C-11 | 연결어미 뒤 쉼표 (-고/-며/-지만 직후 쉼표) | S1 | 쉼표 제거. 6+회=강한 신호 |
...
| E-2 | 동일 종결어미 "~다" 4문장 연속 + "~고 있다" 자동 매핑 | S2 | 다양화. "~고 있다" 단순 시제 환원 |
...
| ID | 패턴 | 심각도 | 처방 |
|---|---|---|---|
| F-4 | 한자어 명사화 -성/-적/-화 + 영어 명사화 누적 12회+ | S2 | 동사·형용사 어근으로 환원 |
...
| I-4 | 권고형 결말 "~해야 한다" 반복 | S2 | 평서·단언으로 |
...
| J-3 | 불릿 리스트 (칼럼·리포트) | S2 | 문단 산문으로 통합 |
...
6. **인공 표현 자제**: 원문에 없던 비유·수사를 임의로 추가하지 않았는가

## 등급 기준

- **A**: S1 잔존 0, S2 ≤2, 변경률 10~25%, 자체검증 6항 모두 통과
- **B**: S1 잔존 0, S2 ≤4, 자체검증 5항 이상 통과
- **C**: S1 1~2 또는 자체검증 4항 이하 — strict 모드 권고
- **D**: S1 3+ 또는 변경률 50% 초과 — 작업 중단 권고
